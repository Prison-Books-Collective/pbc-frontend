...
inmateFunctions = (function () {
  ...
  function createInmateRecord() {
    ...
    fetch(
      `http://localhost:8080/addInmate?firstName=${firstName}&lastName=${lastName}&id=${inmate_id}`,
      {
        method: 'post'
      }
    )
  ...
  function createInmateInfoFields(container, id, firstName, lastName) {
    ...
    container.appendChild(
      helperFunctions.createLabelAndField('First Name', inmateFirstNameField, firstName, '')
    )
    container.appendChild(
      helperFunctions.createLabelAndField('Last Name', inmateLastNameField, lastName, '')
    )
  }
  ...
  function editInmateRecord(originalInmateInfo) {
    ...
    fetch(
      `http://localhost:8080/updateInmate?originalId=${originalInmateInfo.id}&firstName=${firstName}&lastName=${lastName}&id=${inmate_id}`,
      {
        method: 'put'
      }
    )
