inmateNoIDFunctions = (function () {
  ...
  function displaySearchForInmateNoID() {
    ...
    div.appendChild(
      helperFunctions.createLabelAndField('First Name', firstNameFieldId, '', 'Enter first name')
    )
    div.appendChild(
      helperFunctions.createLabelAndField('Last Name', lastNameFieldId, '', 'Enter last name')
    )
  ...
  function createSearchButton() {
    ...
    button.onclick = () => {
      findInmateNoID(
        document.getElementById(firstNameFieldId).value,
        document.getElementById(lastNameFieldId).value
      )
    }
    return button
  }
  ...
  function displayAddInmateNoID(firstName, lastName) {
    ...
    div.appendChild(
      helperFunctions.createLabelAndField('Location: ', locationFieldId, '', 'Enter location')
    )
  ...
  function createInmateNoIDRecord(firstName, lastName, location) {
    fetch(
      `http://localhost:8080/addInmateNoID?firstName=${firstName}&lastName=${lastName}&location=${location}`,
      {
        method: 'post'
      }
    )
