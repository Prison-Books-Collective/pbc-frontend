import { CONTENT_TYPE_JSON, METHOD_GET, METHOD_POST, METHOD_DELETE, METHOD_PUT } from '$lib/util/web'
...
export class PackageService {
  ...
  public static readonly URI_GET_PACKAGES = ( inmateId: string ) => `${ BASE_PBC_URI }/getPackagesForInmate?inmateId=${ inmateId }`
  public static readonly URI_GET_PACKAGES__INMATE_NO_ID = ( database_id: string|number ) => `${ BASE_PBC_URI }/getPackagesForInmateNoId?inmateId=${ database_id }`
  ...
  public static readonly URI_DELETE_PACKAGE = ( packageId: number ) => `${ BASE_PBC_URI }/deletePackage?packageId=${ packageId }`
  public static readonly URI_PACKAGE_COUNT = ( date: string ) => `${BASE_PBC_URI}/getPackageCountFromDate?date=${ date }` // date is a formatted string "yyyy-mm-dd"


  public static async createPackage( pbcPackage: Package ): Promise<Package> {
    const response = await fetch(
      this.URI_CREATE_PACKAGE, {
        ...METHOD_POST,
  ...
  public static async createPackageForInmateNoID( pbcPackage: Package ): Promise<Package> {
    const response = await fetch(
      this.URI_CREATE_PACKAGE__INMATE_NO_ID, {
        ...METHOD_POST,
        headers: { ...CONTENT_TYPE_JSON },
        body: JSON.stringify( pbcPackage ),
      })
