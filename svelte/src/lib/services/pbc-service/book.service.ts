...
export class BookService {
  ...
  public static readonly URI_GET_BOOK__ISBN10 = ( isbn: string ) => `${ BASE_PBC_URI }/getIsbn10?isbn10=${ isbn }`
  public static readonly URI_GET_BOOK__ISBN13 = ( isbn: string ) => `${ BASE_PBC_URI }/getIsbn13?isbn13=${ isbn }`
  ...
  public static async findBook( isbn: string ): Promise<Book|null> {
    ...
    if( isbn.length === 10 ) {
      ...
      throw new Error( `Failed to find book with ISBN ${ isbn }; this value is not a valid ISBN10 or ISBN13 value` )
    }
    ...
    if( response.status !== 200 ) {
      throw new Error( `unexpected response ${ response.status } when searching for book with valid ISBN "${ isbn }" from "${ uri }"` )
    }
  ...
  public static async createBook( book: Book ): Promise<Book> {
    const response = await fetch(
      ...
      if( response.status !== 200 ) {
        throw new Error( `unexpected response ${ response.status } when adding new book to "${ this.URI_CREATE_BOOK }" with details: ${ JSON.stringify( book ) }` )
      }
  ...
  public static async createBookNoISBN( book: NoISBNBook ): Promise<NoISBNBook> {
    const response = await fetch(
      ...
      if( response.status !== 200 ) {
        throw new Error( `unexpected response ${ response.status } when adding new book to "${ this.URI_CREATE_BOOK__NO_ISBN }" with details: ${ JSON.stringify( book ) }` )
      }
  ...
  public static async updateBook( book: Book ): Promise<Book> {
    ...
    if( response.status !== 200 ) {
      throw new Error( `unexpected response ${ response.status } when updating book via "${ this.URI_UPDATE_BOOK }" with details: ${ JSON.stringify( book ) }` )
    }
